...
def performance_on_categorical_slice(
    ...
    the categorical features and a label binarizer
    for the labels. This can be used in either training
    ...
    data : pd.DataFrame
        ...
        Columns in `categorical_features`column_name : str
    ...
    encoder : sklearn.preprocessing._encoders.
        OneHotEncoder Trained sklearn OneHotEncoder,
        only used if training=False.
    lb : sklearn.preprocessing._label.LabelBinarizer
        Trained sklearn LabelBinarizer, only used if
        training=False.
    model : ???
    ...
    if data_slice.shape[0] == 0:
        print(f"No rows for {column_name}={slice_value}")
